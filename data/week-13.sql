...
INSERT INTO "LootAward" ("itemId", "characterId", "rosterId", "raidNightId", "awardedAt") VALUES
  ...
  ((SELECT id FROM "Item" WHERE name = 'Magtheridon''s Head' LIMIT 1), (SELECT c.id FROM "Character" c JOIN "Player" p ON c."playerId" = p.id WHERE p."displayName" = 'Blargn' AND c.class = 'Mage' AND c.spec = 'Frost Mage' LIMIT 1), (SELECT id FROM "Roster" WHERE name = 'Master Roster'), (SELECT id FROM "RaidNight" WHERE date = '2026-05-12' AND "rosterId" = (SELECT id FROM "Roster" WHERE name = 'Master Roster') LIMIT 1), '2026-05-12T00:00:00Z'::timestamp);
